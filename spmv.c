...
int main(int argc, char *argv[])
{
    ...
    timestamp(&now);
    printf("Time taken by LAPACK/OpenBLAS dense computation: %ld ms\n", diff_milli(&start, &now));
    ...
    if (check_result(refsol, mysol, size) == 1)
        printf("Dense result is correct!\n");
    else
        printf("Dense result is incorrect!\n");

    //
    // Let's try now SpMV: Sparse Matrix - Dense Vector computation
    //

    // Using your own sparse implementation
    timestamp(&start);
    ...
    if (check_result(refsol, mysol, size) == 1)
        printf("Sparse result is correct!\n");
    else
        printf("Sparse result is incorrect!\n");
